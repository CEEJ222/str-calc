...
const MobileKpiBar = ({ calculations, formatCurrency, formatPercent, getResultClass }) => {
  if (!calculations) return null;

  return (
    <div className="mobile-kpi-bar">
      {/* Row 1: Assessment */}
      <div className="kpi-row assessment-row">
        <span className="mobile-kpi-label">ASSESSMENT</span>
        <span className={`assessment-value ${calculations.assessmentClass}`}>
          <span className="assessment-emoji" />
          {calculations.assessmentTextShort}
        </span>
      </div>
      ...
      <div className="kpi-row">
        <div className="mobile-kpi-metric">
          <span className="mobile-kpi-label">MONTHLY CF</span>
          <span className={`mobile-kpi-value ${getResultClass(calculations.monthlyCashFlow, {good: 200, ok: 0})}`}>{formatCurrency(calculations.monthlyCashFlow)}</span>
        </div>
        <div className="mobile-kpi-metric">
          <span className="mobile-kpi-label">ANNUAL CF</span>
          <span className={`mobile-kpi-value ${getResultClass(calculations.cashFlow, {good: 2400, ok: 0})}`}>{formatCurrency(calculations.cashFlow)}</span>
        </div>
        <div className="mobile-kpi-metric">
          <span className="mobile-kpi-label">COC RETURN</span>
          <span className={`mobile-kpi-value ${getResultClass(calculations.cashOnCashReturn, {good: 8, ok: 5})}`}>{formatPercent(calculations.cashOnCashReturn)}</span>
        </div>
        <div className="mobile-kpi-metric">
          <span className="mobile-kpi-label">CAP RATE</span>
          <span className={`mobile-kpi-value ${getResultClass(calculations.capRate, {good: 6, ok: 4})}`}>{formatPercent(calculations.capRate)}</span>
        </div>
      </div>

      {/* Row 3: Post-Tax KPIs */}
      <div className="kpi-row post-tax-row">
        <div className="mobile-kpi-metric">
          <span className="mobile-kpi-label">AT MONTHLY CF</span>
          <span className={`mobile-kpi-value ${getResultClass(calculations.afterTaxMonthlyCashFlow, {good: 200, ok: 0})}`}>{formatCurrency(calculations.afterTaxMonthlyCashFlow)}</span>
        </div>
        <div className="mobile-kpi-metric">
          <span className="mobile-kpi-label">AT ANNUAL CF</span>
          <span className={`mobile-kpi-value ${getResultClass(calculations.afterTaxAnnualCashFlow, {good: 2400, ok: 0})}`}>{formatCurrency(calculations.afterTaxAnnualCashFlow)}</span>
        </div>
        <div className="mobile-kpi-metric">
          <span className="mobile-kpi-label">AT COC RETURN</span>
          <span className={`mobile-kpi-value ${getResultClass(calculations.afterTaxCashOnCashReturn, {good: 8, ok: 5})}`}>{formatPercent(calculations.afterTaxCashOnCashReturn)}</span>
        </div>
      </div>
    </div>
  );
};
MobileKpiBar.displayName = 'MobileKpiBar';
...
const AirbnbCalculator = () => {
  ...
  const calculations = useMemo(() => {
    ...
    let assessmentText, assessmentTextShort, assessmentClass;
    if (cashOnCashReturn >= 8 && monthlyCashFlow >= 200) {
      assessmentText = "🟢 Excellent investment opportunity";
      assessmentTextShort = "Excellent";
      assessmentClass = 'good';
    } else if (cashOnCashReturn >= 5 && monthlyCashFlow >= 0) {
      assessmentText = "🟡 Moderate investment potential";
      assessmentTextShort = "Moderate";
      assessmentClass = 'ok';
    } else {
      assessmentText = "🔴 Consider alternative investments";
      assessmentTextShort = "High Risk";
    ...
    return {
      ...
      assessmentText, assessmentTextShort, assessmentClass
    };
  }, [debouncedInputs]);
        ...
        <div className="calculator-wrapper">
          ...
          <div className="calculator-grid">
            ...
            <div className="results-panel">
              {/* Key Metrics */}
              <div className="section main-kpis-section">
                <div className="section-header">
                ...
                <div className="metrics-grid">
                  <div className="metric-card green">
                    ...
                    <div className={`metric-value ${getResultClass(calculations.monthlyCashFlow, {good: 200, ok: 0})}`}>
                      {formatCurrencyShort(calculations.monthlyCashFlow)}
                    </div>
                  </div>
                  ...
                  <div className="metric-card orange">
                    ...
                    <div className={`metric-value ${getResultClass(calculations.cashFlow, {good: 2400, ok: 0})}`}>
                      {formatCurrencyShort(calculations.cashFlow)}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="metrics-grid">
                    <div className="metric-card green">
                      ...
                      <div className={`metric-value ${getResultClass(calculations.afterTaxMonthlyCashFlow, {good: 200, ok: 0})}`}>
                        {formatCurrencyShort(calculations.afterTaxMonthlyCashFlow)}
                      </div>
                    </div>
                    <div className="metric-card blue">
                    ...
                    <div className="metric-card orange">
                      ...
                      <div className={`metric-value ${getResultClass(calculations.afterTaxAnnualCashFlow, {good: 2400, ok: 0})}`}>
                        {formatCurrencyShort(calculations.afterTaxAnnualCashFlow)}
                      </div>
                    </div>
                  </div>
